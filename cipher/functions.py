...
def action_7(request):
    ...
    output_text += enigma(text, int(opt_1), int(opt_2),
                          int(opt_3), opt_4, roter_key, ringsetting_key, plugboard)
...
def action_8(request):
    ...
    if d == 0:
        ...
        output_text += 'calculated d = ' + \
            str(rsa_decode(m, e, n, p, q)[1]) + '<br>'
        output_text += 'RSA Decode: ' + \
            str(rsa_decode(m, e, n, p, q)[0]) + '<br>'
...
def action_15(request):
    ...
    for i in range(count):
        output_text += 'word # ' + \
            str(word_index_list[i]) + \
            ' (' + word_list[word_index_list[i]-1] + ' ),'
...
def action_16(request):
    ...
    for menu in menus:
        output_text += menu + ': ' + \
            table_subtitution(input_text, menu) + '<br>'
...
def action_17(request):
    ...
    if option_1 == '0':
        ...
        for table_description, table in tables.items():
            ...
            output_text += phonetic_alphabet_e(
                input_text_lower, table) + '<br>'
            output_text += '<hr>'
    ...
    if option_1 == '1':
        ...
        for table_description, table in tables.items():
            output_text += table_description + '<br>'
            output_text += return_phonetic_alphabet_values(table) + '<br><br>'
            output_text += phonetic_alphabet_d(
                input_text_lower, table) + '<br>'
            output_text += '<hr>'
...
def action_19(request):
    ...
    output_text += 'Used characters (unique) = ' + \
        unique(input_text, sort=True) + '<br>'
    output_text += '' + '<br>'
...
def action_20(request):
    ...
    output_text += html.escape(str(uu_encode(input_text.encode()))
                               [2:-1]) + '<br>'
    output_text += '' + '<br>'

    output_text += 'ASCII85' + '<br>'
    output_text += html.escape(str(base64.a85encode(input_text.encode()))
                               [2:-1]) + '<br>'
    output_text += '' + '<br>'

    output_text += 'BASE85' + '<br>'
    output_text += html.escape(str(base64.b85encode(input_text.encode()))
                               [2:-1]) + '<br>'
    output_text += '' + '<br>'
...
def action_21(request):
    ...
    output_text = ''
    output_text += 'Input text was interpreted as: ' + \
        ' '.join(input_list) + '<br>'
    output_text += '' + '<br>'
...
def action_22(request):
    ...
    def output_template(byte):
        output_text = ''
        output_text += 'to base16: ' + \
            str(base64.b16encode(byte))[2:-1] + '<br>'
        output_text += 'to base32: ' + \
            str(base64.b32encode(byte))[2:-1] + '<br>'
        output_text += 'to base64: ' + \
            str(base64.b64encode(byte))[2:-1] + '<br>'
        output_text += 'to uuencode: ' + \
            html.escape(str(uu_encode(byte))[2:-1]) + '<br>'
        output_text += 'to ascii85: ' + \
            html.escape(str(base64.a85encode(byte))[2:-1]) + '<br>'
        output_text += 'to base85: ' + \
            html.escape(str(base64.b85encode(byte))[2:-1]) + '<br>'
        output_text += 'to text: ' + html.escape(str(byte)[2:-1]) + '<br>'
        output_text += '<br>'
        return output_text

    input_text_formatted = input_text.upper(
    )[:len(input_text)-(len(input_text) % 2)]
...
def action_24(request):
    ...
    output_text += 'Key strings is interpretted as ' + \
        str(key_assigned) + '<br>'
    output_text += '' + '<br>'
    output_text += 'Periodic Transposition Encode: ' + \
        periodic_transposition_e(input_text, key_assigned) + '<br>'
    output_text += 'Periodic Transposition Decode: ' + \
        periodic_transposition_d(input_text, key_assigned) + '<br>'
...
def action_26(request):
    ...
    output_text += replace_all_case_insensitive(
...
def action_29(request):
    ...
    output_text += 'Text length: ' + \
        str(len(analysys_text)) + ', wo space: ' + \
        str(len(analysys_text_wo_space)) + '<br><br>'
...
def action_30(request):
    ...
    output_text += 'MD5____: ' + \
        hashlib.md5(input_text.encode()).hexdigest() + '<br>'
    output_text += 'SHA1___: ' + \
        hashlib.sha1(input_text.encode()).hexdigest() + '<br>'
    output_text += 'SHA224_: ' + \
        hashlib.sha224(input_text.encode()).hexdigest() + '<br>'
    output_text += 'SHA256_: ' + \
        hashlib.sha256(input_text.encode()).hexdigest() + '<br>'
    output_text += 'SHA384_: ' + \
        hashlib.sha384(input_text.encode()).hexdigest() + '<br>'
    output_text += 'SHA512_: ' + \
        hashlib.sha512(input_text.encode()).hexdigest() + '<br>'
    output_text += 'BLAKE2b: ' + \
        hashlib.blake2b(input_text.encode()).hexdigest() + '<br>'
    output_text += 'BLAKE2s: ' + \
        hashlib.blake2s(input_text.encode()).hexdigest() + '<br>'
    output_text += '' + '<br>'
